...
<div class="container">
	...
	<div class="row">
		...
		<div class="row">
			...
			<h4 class="text-center">Статистика по площадкам по товарным виджетам в период с {{date('d-m-Y',strtotime($from))}} по {{date('d-m-Y',strtotime($to))}}</h4>
			<div class="col-xs-12">
			{!! $allStat->appends(["from"=>$from, "order"=>$order, "direct"=>$direct, 'number'=>$number, "to"=>$to, "manager"=>$manager])->render() !!}<br>
			    ...
			    <div class="tab-content">
					...
					@include('mp-statistica::product.pads.all',['header'=>$header,'summaryStats'=>$allStat,'summary'=>$allAll])
					</div>	
					<div class="tab-pane" id="yandex_stat" style="margin-top: 10px;">
					@include('mp-statistica::product.pads.yandex',['header'=>$header,'summaryStats'=>$yandexStat,'summary'=>$allYandex])
					</div>		
					<div class="tab-pane" id="ta_stat" style="margin-top: 10px;">
					@include('mp-statistica::product.pads.topadvert',['header'=>$header,'summaryStats'=>$topStat,'summary'=>$allTop])
					</div>	
					<div class="tab-pane" id="my_stat" style="margin-top: 10px;">
					@include('mp-statistica::product.pads.my',['header'=>$header,'summaryStats'=>$myStat,'summary'=>$allMy])
					</div>						
			    </div>		
